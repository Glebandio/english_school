...
function openModalWindow() {
	...
	let widthScrollLine = widthScroll();
	window.addEventListener("resize", () => {
		widthScrollLine = widthScroll();
	});
	...
	if (btn && modalContainer) {
		...
		btn.forEach((item) => {
			item.addEventListener("click", (e) => {
				...
				addStylePage(widthScrollLine);
				modalContainer.forEach((e) => {
					e.classList.remove("--active");
				});
				idBox.classList.add("--active");
				body.classList.add("--stop-modal");
			});
		});
		...
		function close() {
			body.classList.remove("--stop-modal");
			let styleWrapper = document.querySelector(".style--wrapper");
			if (styleWrapper) {
				document.body.removeChild(styleWrapper);
			}
...
function widthScroll() {
	...
	let mainWidth = width.clientWidth;
	let windowWidth = window.innerWidth;
	return windowWidth - mainWidth;
}

function addStylePage(widthScrollLine) {
	const scrollDiv = document.createElement("div");
	scrollDiv.classList.add("style--wrapper");
	scrollDiv.innerHTML = `<style> .--stop-modal {
					margin-right: ${widthScrollLine}px;
				}</style>`;
...
function fixedMenu() {
	...
	if (header && main) {
		...
		const mainHeigth = main.offsetTop + 5;
